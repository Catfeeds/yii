...
class ProductUpdate extends namespace\base\ProductUpdate
{
    public function rules() {
        $rules = parent::rules();
        $childRules = [
            [['product_id', 'product_category_id', 'barcode', 'supplier_id', 'supplier_product_id', 'sale_price'], 'required', 'message' => "{attribute}不能为空"],
    ...
    public function addUpdate($model, $post) {
        if($post["Product"]["product_category_id"] == $model->product_category_id && $post["Product"]["barcode"] == $model->barcode &&
