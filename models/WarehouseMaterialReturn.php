            ...
            foreach ($materialReturnProduct as $productVal) {
                ...
                if($stockOutItem) {
                    ...
                    if($this->buying_id) {
                        ...
                        $stockInItem->number = $num;
                        $stockInItem->warehouse_id = $this->warehouse_id;
                        $stockInItem->supplier_id = $this->supplier_id;
                        $stockInItem->type = $productVal->type;
                        if(!$stockInItem->save()) {
                            return ["state" => 0, "message" => $stockInItem->getFirstErrors()];
                        }
                        $result = WarehouseGateway::addWarehouseGateway($this->warehouse_id, $productVal->product_id, WarehouseGateway::TYPE_IN, 0, $num, $this->id, WarehouseGateway::GATEWAY_TYPE_BUYING, $productVal->type, $stockInItem->batches);
                        if(!$result["state"]) {
                            return $result;
                        }
                        continue;
                    } else {
                        return ["state" => 0, "message" => "退货逻辑错误：未找到商品：".$productVal->name."的库存，无法退货！"];
                    }
                }
            }
        }
        $model = new DepartmentBalanceLog();
        $result = $model->addDepartmentBalanceLog($this->department_id, $this->id, DepartmentBalanceLog::BUSINESS_TYPE_MATERIAL_RETURN, DepartmentBalanceLog::MOD_IN, $this->total_amount, "物料退货");
        if(!$result["state"]) {
            return $result;
        }
