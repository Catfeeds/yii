        ...
        try{
            ...
            foreach ($post["stockId"] as $key => $stockId) {
                ...
                if($post["goodsNum"][$key] > $stockItem->number){
                    $transaction->rollBack();
                    return array("state" => 0, "message" => $productItem->name."的退出数量不能大于库存");
                }
                ...
                $backProduct->purchase_price = $stockItem->type == WarehousePlanning::TYPE_EXCEPTION ? $productItem->price : $productItem->purchase_price;
                ...
                $backProduct->total_amount = $backProduct->sale_price * $backProduct->buying_number;
