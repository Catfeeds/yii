...
$nextStep = Flow::showNextStepByInfo(Flow::TYPE_ORDER_MATERIAL, $data);
$allStep = Flow::showAllStep(Flow::TYPE_ORDER_MATERIAL, $data);
?>
<tr>
    <td><?= $key+1 ?></td>
    ...
    <td>
        <a class="quick-form-button" href="<?= Url::to(['omaterialreturn/info',"id" => $data->id]) ?>">详情</a>
        <?php foreach ($allStep as $stepVal) { ?>
            <?php if($stepVal["state"]) { ?>
                <a class="quick-form-button" href="<?= Url::to(['omaterialreturn/info',"id" => $data->id]) ?>" style="margin-left: 10px;"><?php echo $stepVal["stepName"];?></a>
            <?php } else {  ?>
        ...
        <?php if($data->status != Flow::STATUS_FINISH || $data->pay_state == $data::PAY_STATUS_ALL) { ?>
                <span style="margin-left: 10px;">收款</span>
        <?php } else  {?>
                <a class="quick-form-button" href="<?= Url::to(['omaterialreturn/info',"id" => $data->id]) ?>" style="margin-left: 10px;">收款</a>
        <?php } ?>
    </td>
</tr>