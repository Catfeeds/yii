...
<div class="main-container">
    <div class="filter">
        <form method="get">
            <input type="hidden" name="r" value="order/ajaxdepartproductlist" />
            <span>物料名称
            ...
            <span>供应商
                <input class="form-text verifySpecial" type="text" placeholder="供应商名称..." name="supplier_name" value="<?= Yii::$app->request->get('supplier_name') ?>" onkeyup="javascript:validateValue(this)"/>
            </span>
            <span>物料类别
            ...
            <tr>
                ...
                <td><input type="type" class="selGoodsNum_<?= $data->id ?>" value="" onkeyup="value=value.replace(/\D/g,'')"/>
                    <input type="hidden" class="productId_<?= $data->id ?>" value="<?= $data->product_id ?>" /></td>
                     <input type="hidden" class="warehouseId_<?= $data->id ?>" value="<?= $data->warehouse_id ?>" />
            </tr>
        <?php } else {
    ...
    $(function () {
            ...
            $(".selGoods").each(function(index){
                if($(this).attr("checked") == "checked") {
                    ...
                    selData[n]["selGoodsNum"] = $(".selGoodsNum_"+i).val();
                     selData[n]["warehouseId"] = $(".warehouseId_"+i).val();
                    isSub = true;
                    n++;
                }
            });
        ...
        $(".selectGoods").click(function(){
            ...
            selData[0]["selGoodsNum"] = $(".selGoodsNum_"+i).val();
             selData[0]["warehouseId"] = $(".warehouseId_"+i).val();
            window.parent.func(selData);  
        });
    });
</script>
