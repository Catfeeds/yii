...
<div class="main-container">
    <div class="filter">
        <form method="get">
            <input type="hidden" name="r" value="departmentcheckplanning/index" />
            <span>计划名称
    ...
    <table id="table-list" class="table-list">
        ...
        <?php if($listDatas){ 
            foreach($listDatas as $key => $data){
                $nextStep = Flow::showNextStepByInfo(Flow::TYPE_CHECK_DEPARTMENT, $data);
                $allStep = Flow::showAllStep(Flow::TYPE_CHECK_DEPARTMENT, $data);
        ?>
            <tr>
                ...
                <td>
                    <a href="<?= Url::to(['departmentcheckplanning/info', 'id' => $data->id]) ?>">详情</a>
                    <?php foreach ($allStep as $stepVal) { ?>
                        <?php if($stepVal["state"]) { ?>
                            <a class="quick-form-button" href="<?= Url::to(['departmentcheckplanning/info',"id" => $data->id]) ?>" style="margin-left: 10px;"><?php echo $stepVal["stepName"];?></a>
                        <?php } else {  ?>
                            <span style="margin-left: 10px;"><?php echo $stepVal["stepName"];?></span>
                        <?php } ?>
                    <?php } ?>
                    <?php if($data->status == Flow::STATUS_FINISH && !$data->is_proof){ ?>
                           <a class="quick-form-button" href="<?= Url::to(['departmentcheckplanning/proof',"id" => $data->id]) ?>" style="margin-left: 10px;">盘点</a> 
                    <?php } else { ?>
        ...
        <?php } } else { ?>
            <tr><td colspan="13">暂无部门盘点计划</td></tr>
        <?php } ?>
    </table>
    ...
    <?php ActiveForm::end(); ?>
    <div class="buttons">
        <a class="button blue-button" href="<?= Url::to(['departmentcheckplanning/addorupdate']) ?>">新增</a>
