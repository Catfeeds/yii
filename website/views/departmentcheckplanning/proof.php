...
<div class="main-container">
    ...
    <?= Html::activeHiddenInput($checkFlow, 'department_id', ['value' => $model->department_id])?>
    <table id="table-list" class="table-list taleft">
        <caption>盘点的部门列表</caption>
        <?php if($model->is_check_amount){ ?>
        <tr class="amountTr_<?= $model->department_id;?>">
            <td style="width:20%;"><span style="font-size:18px;color: red;"><?= $departmentName ?> --> 盘点资金</span></td>
            <td style="width:30%;">部门余额：<span class="showAmount"><?= isset($checkInfo["amountList"][$model->department_id]) ? $checkInfo["amountList"][$model->department_id] : "0"?></span></td>
            <td style="width:30%;">真实余额：
                <input type="text" name="checkAmount[<?= $model->department_id;?>]" class='checkAmount verifyMinus' style="width: 60%;" i="<?= $model->department_id;?>"></td>
            <td style="width:20%;">结存差额：<span class="showAmountDiff"><?= isset($checkInfo["amountList"][$model->department_id]) ? $checkInfo["amountList"][$model->department_id] : "0"?></span></td>
        </tr>
        <?php } ?>
    ...
    <div class="buttons">
        ...
        <a class="button blue-button confirmChcek" href="javascript:void(0)" save-data="<?= Url::to(['departmentcheckplanning/proof', 'id' => $model->id]) ?>">保存</a> 
        <a class="button blue-button" href="javascript:history.back(-1)">返回</a>
        <a class="button blue-button" href="<?= Url::to(['departmentcheckplanning/downproof', 'id' => $model->id]) ?>" >导出</a>
        <a class="button blue-button" import-excel="<?= Url::to(['departmentcheckplanning/importproof', 'id' => $model->id]) ?>"  href="javascript:void(0)">导入</a>
