...
$nextStep = Flow::showNextStepByInfo(Flow::TYPE_CHECKOUT, $data);
$allStep = Flow::showAllStep(Flow::TYPE_CHECKOUT, $data);
?>
<tr>
    ...
    <td>
        <a class="quick-form-button" href="<?= Url::to(['wcheckout/info',"id" => $data->id]) ?>">详情</a>
        <?php foreach ($allStep as $stepVal) { ?>
            <?php if($stepVal["state"]) { ?>
                <a class="quick-form-button" href="<?= Url::to(['wcheckout/info',"id" => $data->id]) ?>" style="margin-left: 10px;"><?php echo $stepVal["stepName"];?></a>
            <?php } else {  ?>
