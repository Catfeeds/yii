...
$nextStep = Flow::showNextStepByInfo(Flow::TYPE_TRANSFEF, $data);
$allStep = Flow::showAllStep(Flow::TYPE_TRANSFEF, $data);
?>
<tr>
    <td><?= $key + 1; ?></td>
    ...
    <td>
        <a class="quick-form-button" href="<?= Url::to(['wtransfer/info',"id" => $data->id]) ?>">详情</a>
        <?php foreach ($allStep as $stepVal) { ?>
            <?php if($stepVal["state"]) { ?>
                <a class="quick-form-button" href="<?= Url::to(['wtransfer/info',"id" => $data->id]) ?>" style="margin-left: 10px;"><?php echo $stepVal["stepName"];?></a>
            <?php } else {  ?>
