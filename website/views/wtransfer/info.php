...
$this->title = '业务操作-物料调仓详情';
$nextStep = Flow::showNextStepByInfo(Flow::TYPE_TRANSFEF, $model, 'operator_url');
$remarkList = CommonRemark::getRemarkList($model->id, Flow::TYPE_TRANSFEF);
?>
<?= $this->context->renderPartial('/public/menu') ?>
<div class="main-container">
    <div class="filter">
        <table id="table-list" class="table-list taleft">
            <caption>物料调仓详情</caption>
            <tr id="quick-form">
                <td width="30%">调仓名称：<?= $model->name; ?></td>
                <td width="25%">调出仓库：<?= Warehouse::getNameById($model->warehouse_id) ?></td>
                <td width="25%">调入仓库：<?= Warehouse::getNameById($model->receive_warehouse_id) ?></td>
                <td width="20%">进展状态：<?= Flow::showStatusAll($model->status) ?></td>
            </tr>
            <tr>
                <td>调仓编号：<?= $model->sn ?></td>
                <td>调仓总价：<?= number_format($model->total_amount, 2) ?></td>
            ...
            <tr>
                <td colspan="4">调仓说明：<?= $model->remark ? $model->remark : "无"  ?></td>
            </tr>
            <tr>
