...
<div class="main-container">
    <div class="filter">
        <form method="get">
            <input type="hidden" name="r" value="checkproof/index" />
            <span>校队名称
    ...
    <table id="table-list" class="table-list">
        ...
        <?php if($listDatas){ 
            foreach($listDatas as $key => $data){
                $nextStep = Flow::showNextStepByInfo(Flow::TYPE_CHECK_PLANNING_PROOF, $data);
                $allStep = Flow::showAllStep(Flow::TYPE_CHECK_PLANNING_PROOF, $data);
        ?>
            <tr>
                ...
                <td>
                    <a href="<?= Url::to(['checkproof/info', 'id' => $data->id]) ?>">详情</a>
                    <?php foreach ($allStep as $stepVal) { ?>
                        <?php if($stepVal["state"]) { ?>
                            <a class="quick-form-button" href="<?= Url::to(['checkproof/info',"id" => $data->id]) ?>" style="margin-left: 10px;"><?php echo $stepVal["stepName"];?></a>
                        <?php } else {  ?>
        ...
        <?php } } else { ?>
            <tr><td colspan="12">暂无总盘点计划校队数据</td></tr>
        <?php } ?>
    </table>
