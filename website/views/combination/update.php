...
<div class="main-container">
                ...
                <table class="showGoodsList" style="width: 100%" border="1">
                    ...
                    <?php foreach($info as $key => $data){ ?>
                        <tr class="showGoodsTr tr_<?= $key+1 ?>" i="1">
                            ...
                            <td><?= Html::input("text", "goodsNum[]", $data->product_number,["class" => "selGoodsNum", "onblur"=>"javascript:ckprto(".($key + 1).")", "onkeyup"=>"value=value.replace(/\D/g,'')"]);?></td>
                            <td><span class="goodsTotalMoney"><?= number_format($data->total_amount, 2) ?></span></td>
                            <td>
                                <?= Html::hiddenInput("goodsId[]", $data->pstock_id ,["class" => "selGoodsId"])?>
   ...
   <div class="buttons">
      <a class="button blue-button" href="javascript:void(0)" save-data="<?= Url::to(['combination/update', 'id' => $model->id]) ?>">保存</a> 
...
<?= $this->context->renderPartial('/jquery/warehouseProductStock', compact("num")) ?>
