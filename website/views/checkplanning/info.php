...
$this->title = '业务操作-总盘点计划详情';
$nextStep = Flow::showNextStepByInfo(Flow::TYPE_CHECK_PLANNING, $model, 'operator_url');
$remarkList = CommonRemark::getRemarkList($model->id, Flow::TYPE_CHECK_PLANNING);
?>
<?= $this->context->renderPartial('/public/menu') ?>
<div class="main-container">
    <div class="filter">
        <table id="table-list" class="table-list taleft">
            <caption>总盘点计划详情</caption>
            <tr id="quick-form">
                ...
                <td width="20%">结束盘点时间：<?= $model->end_time ?></td>
            </tr>
            <tr>
                <td>流程名称：<?= FlowConfig::getNameById($model->config_id) ?></td>
                <td>是否盘点资金：<?= $model->is_check_amount ? "是" : "否" ?></td>
                <td colspan="2">盘点商品名称：<?= $model->product_name ? $model->product_name : "全部" ?></td>
            </tr>
            <tr>
                <td>盘点编号：<?= $model->sn ?></td>
                ...
                <td>盘点供应商：<?= $model->supplier_id ? Supplier::getNameById($model->supplier_id) : "全部" ?></td>
            </tr>
            <tr>
    ...
    <div class="buttons">
        ...
        <?php if($model->status == Flow::STATUS_FINISH && !$model->is_proof){ ?>
            <a class="button blue-button" href="<?= Url::to(['checkplanning/proof',"id" => $model->id]) ?>">盘点</a> 
         <?php } ?>
