...
<div class="main-container">
    <div class="filter">
        <form method="get">
            <input type="hidden" name="r" value="warehousecheckproof/index" />
            <span>校队名称
    ...
    <table id="table-list" class="table-list">
        ...
        <tr>
            ...
            <th width="5%">盘点人</th>
            <th width="8%">盘点仓库</th>
            <th width="10%">盘点前<br>物料金额</th>
            <th width="10%">盘点后<br>物料金额</th>
            <th width="10%">状态</th>
            <th width="10%">流程名</th>
        ...
        <?php if($listDatas){ 
            foreach($listDatas as $key => $data){
                ...
                $allStep = Flow::showAllStep(Flow::TYPE_CHECK_WAREHOUSE_PROOF, $data);
        ?>
            <tr>
                ...
                <td>
                    <a href="<?= Url::to(['warehousecheckproof/info', 'id' => $data->id]) ?>">详情</a>
                    <?php foreach ($allStep as $stepVal) { ?>
                        <?php if($stepVal["state"]) { ?>
                            <a class="quick-form-button" href="<?= Url::to(['warehousecheckproof/info',"id" => $data->id]) ?>" style="margin-left: 10px;"><?php echo $stepVal["stepName"];?></a>
                        <?php } else {  ?>
