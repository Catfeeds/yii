...
$this->title = '业务操作-物料转货详情';
$nextStep = Flow::showNextStepByInfo(Flow::TYPE_TRANSFEFDEP, $model, 'operator_url');
$remarkList = CommonRemark::getRemarkList($model->id, Flow::TYPE_TRANSFEFDEP);
?>
<?= $this->context->renderPartial('/public/menu') ?>
<div class="main-container">
    <div class="filter">
        <table id="table-list" class="table-list taleft">
            <caption>物料转货详情</caption>
            <tr id="quick-form">
                <td width="30%">转货名称：<?= $model->name; ?></td>
                <td width="15%">进展状态：<?= Flow::showStatusAll($model->status) ?></td>
                <td width="15%">转出仓库：<?= Warehouse::getNameById($model->warehouse_id) ?></td>
                <td width="20%">转入仓库：<?= Warehouse::getNameById($model->receive_warehouse_id) ?></td>
                <td width="20%">是否扣仓：<?= Flow::showBuckleName($model->is_buckle) ?></td>
            </tr>
            <tr>
                <td>转货编号：<?= $model->sn ?></td>
                <td>供应商：<?= Supplier::getNameById($model->supplier_id) ?></td>
                <td>转货总成本：<?= number_format($model->total_cost, 2) ?></td>
                <td>转货总价：<?= number_format($model->total_amount, 2) ?></td>
            ...
            <tr>
                <td colspan="5">退货说明：<?= $model->remark ? $model->remark : "无"  ?></td>
            </tr>
            <tr>
