...
<div class="main-container">
    ...
    <table id="table-list" class="table-list taleft">
        <caption>新增退货申请</caption>
        <tr id="quick-form">
            <td style="width:30%">表单名：<?= Html::activeTextInput($model, 'name', ['style' => 'width:70%',"onkeyup" => "javascript:validateValue(this)", 'class' => "verifySpecial"]) ?></td>
            <td style="width:25%">退货仓库：<?= Html::activeDropDownList($model, 'warehouse_id', Warehouse::getAllByStatus(Warehouse::STATUS_OK, '', $departmentId), ['prompt' => '请选择', 'class' => "selWarehouse"]) ?></td>
            <td style="width:25%">供应商：<?= Html::activeDropDownList($model, 'supplier_id', Supplier::getSupplierSelectData(), ['prompt' => '请选择', 'class' => "selSupplier"]) ?></td>
            <td style="width:20%">退货总价：<span class="totalAmount">0.00</span></td>
        </tr>
        <tr>
            <td>表单号：<?= Html::activeTextInput($model, 'sn', ['style' => 'width:70%', 'value'=> Utils::generateSn(Flow::TYPE_MATERIALRETURN),"onkeyup" => "javascript:validateValue(this)", 'class' => "verifySpecial"]) ?></td>
            <td>退货时间：<?= Html::activeTextInput($model, 'planning_date', ['style' => 'width:50%', 'class' => "selDate", "readonly" => "readonly"]) ?></td>
            <td>退款时间：<?= Html::activeTextInput($model, 'payment_term', ['style' => 'width:50%', 'class' => "selDate", "readonly" => "readonly"]) ?></td>
   ...
   <div class="buttons">
      <a class="button blue-button" href="javascript:void(0)" save-data="<?= Url::to(['wmaterial/add']) ?>">保存</a> 
...
<?= $this->context->renderPartial('/jquery/warehouseSupplierProductStock') ?>
<script>
