...
$this->title = '业务操作-物料退货详情';
$nextStep = Flow::showNextStepByInfo(Flow::TYPE_MATERIALRETURN, $model, 'operator_url');
$remarkList = CommonRemark::getRemarkList($model->id, Flow::TYPE_MATERIALRETURN);
?>
<?= $this->context->renderPartial('/public/menu') ?>
<div class="main-container">
    <div class="filter">
        <table id="table-list" class="table-list taleft">
            <caption>物料退货详情</caption>
            <tr id="quick-form">
                <td width="30%">退货名称：<?= $model->name; ?></td>
                <td width="18%">进展状态：<?= Flow::showStatusAll($model->status) ?></td>
                <td width="18%">退货仓库：<?= Warehouse::getNameById($model->warehouse_id) ?></td>
                <td width="18%">退货总价：<?= number_format($model->total_amount, 2) ?></td>
            ...
            <tr>
                ...
                <td colspan="2">预计退款时间：<?= $model->payment_term ?></td>
            </tr>
            <tr>
                <td colspan="5">退货说明：<?= $model->common ? $model->common : "无"  ?></td>
            </tr>
            <tr>
