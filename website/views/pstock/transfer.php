...
<div class="main-container">
    ...
    <table id="table-list" class="table-list taleft">
        <caption>调仓申请</caption>
        <tr id="quick-form">
            <td style="width:30%;">
                表单名：<?= Html::activeTextInput($model, 'name', ['style' => 'width:70%',"onkeyup" => "javascript:validateValue(this)", 'class' => 'verifySpecial']) ?></td>
            <td style="width:30%;">
                调仓单号：<?= Html::activeTextInput($model, 'sn', ['style' => 'width:50%', 'value'=>Utils::generateSn(Flow::TYPE_TRANSFEF),"onkeyup" => "javascript:validateValue(this)", 'class' => 'verifySpecial']) ?></td>
            <td style="width:20%;">
                调出仓库：<?= Html::activeDropDownList($model, 'warehouse_id', Warehouse::getAllByStatus(Warehouse::STATUS_OK, Warehouse::SALE_NO,$departmentId), ['prompt' => '请选择', 'class' => "selWarehouse"]) ?></td>
            <td style="width:20%;">
                调入仓库：<?= Html::activeDropDownList($model, 'receive_warehouse_id', Warehouse::getAllByStatus(Warehouse::STATUS_OK, Warehouse::SALE_NO,$departmentId), ['prompt' => '请选择']) ?></td>
        </tr>
        <tr>
            <td>调动总价：<span class="totalAmount">0.00</span></td>
            <td>是否扣仓：<?= Html::activeDropDownList($model, 'is_buckle', Flow::getBuckleSelectData()) ?></td>
            <td colspan="2">调仓说明：<?= Html::activeTextInput($model, 'remark', ['style' => 'width:70%',"onkeyup" => "javascript:validateValue(this)", 'class' => 'verifySpecial']) ?></td>
        </tr>
   ...
   <div class="buttons">
      <a class="button blue-button" href="javascript:void(0)" save-data="<?= Url::to(['pstock/transfer']) ?>">保存</a> 
