...
<div class="main-container">
    ...
    <table id="table-list" class="table-list taleft">
        <caption>转货申请</caption>
        <tr id="quick-form">
            <td style="width:30%">表单名：<?= Html::activeTextInput($model, 'name', ['style' => 'width:60%', "onkeyup" => "javascript:validateValue(this)", "maxlength" => 20, 'class' => 'verifySpecial']) ?></td>
            <td style="width:25%">转出仓库：<?= Html::activeDropDownList($model, 'warehouse_id', Warehouse::getAllByStatus(Warehouse::STATUS_OK, Warehouse::SALE_YES, $departmentId), ['prompt' => '请选择', 'class' => "selWarehouse"]) ?></td>
            <td style="width:25%">转入仓库：<?= Html::activeDropDownList($model, 'receive_warehouse_id', Warehouse::getAllByStatus(Warehouse::STATUS_OK, Warehouse::SALE_YES, $departmentId), ['prompt' => '请选择']) ?></td>
            <td style="width:20%">是否扣仓：<?= Html::activeDropDownList($model, 'is_buckle', Flow::getBuckleSelectData()) ?></td>
        </tr>
        <tr>
            <td colspan="2">表单号：<?= Html::activeTextInput($model, 'sn', ['style' => 'width:60%', 'value'=>Utils::generateSn(Flow::TYPE_TRANSFEFDEP),"onkeyup" => "javascript:validateValue(this)", 'class' => 'verifySpecial']) ?></td>
            <td colspan="2">供应商：<?= Html::activeDropDownList($model, 'supplier_id', Supplier::getSupplierSelectData(), ['prompt' => '请选择', 'class' => "selSupplier"]) ?></td>
        </tr>
            ...
            <td colspan="4">
                <table class="showGoodsList" style="width: 100%" border="1">
                    <tr>
                        ...
                        <th width="5%">单位</th>
                        <th width="5%">库存</th>
                        <th width="10%">转动<br>数量</th>
                        <th width="8%">转动<br>成本总价</th>
                        <th width="8%">转动<br>销售总价</th>
                        <th width="5%">操作</th>
                    </tr>
                    <?php for ($i = 1; $i < 6; $i++) { ?>
        ...
        <tr>
            <td>转货采购总价：<span class="totalAmount">0.00</span></td>
            <td>转货销售总价：<span class="totalSaleAmount">0.00</span></td>
            <td>制表人：<?= Yii::$app->user->getIdentity()->username; ?></td>
            <td>制表时间：<?= date("Y-m-d H:i:s") ?></td>
        </tr>
        <tr>
            <td colspan="4">转货说明：<?= Html::activeTextInput($model, "remark" ,['style' => "width:80%", 'class' => 'verifySpecial'])?></td>
        </tr>
    </table>
   <div class="buttons">
      <a class="button blue-button" href="javascript:void(0)" save-data="<?= Url::to(['pstock/transferdep']) ?>">保存</a> 
...
<?php $type = 'transferdep';?>
<?= $this->context->renderPartial('/jquery/warehouseSupplierProductStock', compact("type")) ?>
<script>
