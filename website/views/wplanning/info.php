...
$this->title = '业务操作-物料采购计划详情';
$nextStep = Flow::showNextStepByInfo(Flow::TYPE_PLANNING, $model, 'operator_url');
$remarkList = CommonRemark::getRemarkList($model->id, Flow::TYPE_PLANNING);
?>
<?= $this->context->renderPartial('/public/menu') ?>
<div class="main-container">
    <table id="table-list" class="table-list taleft">
        <caption>物料采购计划详情</caption>
        <tr id="quick-form">
            <td style="width: 25%;">计划名称：<?= $model->name; ?></td>
            <td style="width: 35%;">计划编号：<?= $model->sn ?></td>
            <td style="width: 20%;">流程名称：<?= FlowConfig::getNameById($model->config_id) ?></td>
            <td style="width: 20%;">采购总价：<?= number_format($model->total_money, 2) ?></td>
        </tr>
        <tr>
            <td>付款时间：<?= $model->payment_term ?></td>
            <td>支付方式：<?= $model->showPayment() ?></td>
            <td>定金：<?= number_format($model->deposit, 2) ?></td>
            <td>扣项：<?= number_format($model->buckle_amount, 2) ?></td>
        </tr>
        <tr>
            <td>计划下单时间：<?= $model->planning_date ?></td>
            <td>仓库：<?= Warehouse::getNameById($model->warehouse_id) ?></td>
