...
$departmentId = 0;
$this->title = '销存管理-实时销存列表';
?>
<?= $this->context->renderPartial('/public/menu') ?>
<div class="main-container">
    <div class="filter">
        <form method="get">
            <input type="hidden" name="r" value="invoicingsale/index" />
            <span>实时销存名称
            ...
            <span>状态
                <?= Html::dropDownList('status', Yii::$app->request->get('status'), ProductInvoicingSale::getStatusSelectData(), ['prompt' => '请选择', 'class' => 'form-select']) ?>
            </span><br>
            <span>开始时间
    ...
    <table id="table-list" class="table-list">
        <caption>物料实时销存订单列表</caption>
        <tr>
            <th width="3%">序号</th>
            <th width="10%">创建时间</th>
            <th width="10%">实时销存名称</th>
            <th width="6%">实时销存仓库</th>
            <th width="5%">实时销存总价</th>
            <th width="5%">制表人</th>
            <th width="5%">进展状态</th>
        ...
        <?php } } else { ?>
        <tr><td colspan="10">暂无符合条件的物料实时销存记录</td></tr>
        <?php } ?>
    </table>
    <?= LinkPager::widget([
    ...
    <div class="buttons">
        <a class="button blue-button" href="<?= Url::to(['pstock/invoicingsale']) ?>">新增实时销存</a>
