...
DateInput.prototype = {
  ...
  selectMonth: function(date) {
    ...
    if (!this.currentMonth || !(this.currentMonth.getFullYear() == newMonth.getFullYear() &&
      ...
      var type = this.input.attr("i"); //1：最大的有效时间为当前时间 2：最小的有效时间为当前时间
      // Draw each of the days
      for (var i = 0; i <= numDays; i++) {
        var currentDay = new Date(rangeStart.getFullYear(), rangeStart.getMonth(), rangeStart.getDate() + i, 12, 00);
        
        if (this.isFirstDayOfWeek(currentDay)) dayCells += "<tr>";
        var checkValid = this.checkValidFun(type, newData, currentDay);
        if (currentDay.getMonth() == date.getMonth()) {
          dayCells += '<td class="'+(checkValid ? 'unselected_month' : 'selectable_day')+'" date="' + this.dateToString(currentDay) + '" style="text-align:center;padding:0;">' + currentDay.getDate() + '</td>';
        } else {
          dayCells += '<td class="unselected_month" date="' + this.dateToString(currentDay) + '" style="text-align:center;padding:0;">' + currentDay.getDate() + '</td>';
        };
        
        if (this.isLastDayOfWeek(currentDay)) dayCells += "</tr>";
      };
      dayCells += "<tr><td colspan='3' class='selectable_day' date='"+this.dateToString(newData)+"' style='text-align:center;padding:0;'>今天</td>";
      dayCells += "<td colspan='4' class='selectable_day' date='' style='text-align:center;padding:0;'>清除</td></tr>";
  ...
  checkValidFun: function (type, newData, currentData) {
      if(type == 1) {
          if(newData.getYear() < currentData.getYear()) {
              return true;
          } 
          if(newData.getYear() > currentData.getYear()) {
              return false;
          } 
          if(newData.getMonth() < currentData.getMonth()) {
              return true;
          } 
          if(newData.getMonth() > currentData.getMonth()) {
              return false;
          } 
          return newData.getDate() < currentData.getDate() ? true : false;
      }
      if(newData.getYear() > currentData.getYear()) {
        return true;
      } 
