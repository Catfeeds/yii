 ...
 * 业务操作 - 盘点计划管理
 */
class CheckplanningController extends CController
{    
    public function init() {
        $this->checkCheckOverdue();
        parent::init();
    }
    /**
     * 盘点计划列表页
    ...
    {
        if(!Admin::checkSupperAdmin() && !RoleJurisdiction::checkRoleJue(Yii::$app->requestedRoute)){
        ...
        if(Yii::$app->request->post()) {
            $result = $model->addOrUpdateCheckPlanning(Yii::$app->request->post(), CheckPlanningFlow::TYPE_PLANNING);
            if($result["state"]) {
                $return["message"] = "新增成功";
                $return["type"] = "url";
                $return["url"] = Url::to(["checkplanning/index"]);
    ...
    public function actionVerify($id) 
    {
        ...
        $result = Flow::Verify(Flow::TYPE_CHECK_PLANNING, $model, $remark);
    ...
    public function actionApproval($id)
    {
        $model = CheckPlanningFlow::findOne($id);
        $remark = Yii::$app->request->get('remark');
        $result = Flow::Approval(Flow::TYPE_CHECK_PLANNING, $model, $remark);
        return Json::encode($result);
    }
    ...
    public function actionFinish($id)
    {
        $model = CheckPlanningFlow::findOne($id);
        $remark = Yii::$app->request->get('remark');
        $result = Flow::Finish(Flow::TYPE_CHECK_PLANNING, $model, $remark);
        return Json::encode($result);
    }
    ...
    public function actionReject()
    {
        ...
        $model = CheckPlanningFlow::findOne($id);
        $result = Flow::Reject(Flow::TYPE_CHECK_PLANNING, $model, $failCause);
        return Json::encode($result);
    }
    ...
    public function downloadIndex($query) {
        ...
        foreach ($all as $key => $val) {
            $nextStep = Flow::showNextStepByInfo(Flow::TYPE_CHECK_PLANNING, $val);
            $datas[] = [
                'key' => $key+1,
                "name" => $val->name,
                "sn" => $val->sn,
                ...
                "nextStep" => $val->status == Flow::STATUS_FINISH && !$val->is_proof ? "盘点校对" : (isset($nextStep["nextStep"]) ? $nextStep["nextStep"] : "无"),
        ...
        return Utils::downloadExcel($datas, $columns, "总盘点计划列表");
    }
    ...
    public function actionProof($id) {
        ...
        if(Yii::$app->request->post()) {
            ...
            if($result["state"]) {
                Yii::$app->cache->delete("importList_".$id);
                $return["message"] = "申请成功";
                $return["type"] = "url";
                $return["url"] = Url::to(["checkproof/index"]);
                return Json::encode($return);
            }
            $return['message'] = is_array($result["message"]) ? reset($result["message"]) : $result["message"];
            return Json::encode($return);
