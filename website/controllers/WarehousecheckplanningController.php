 ...
 * 业务操作 -- 库存管理 -- 仓库盘点计划
 */
class WarehousecheckplanningController extends CController 
{
    /**
     * 仓库盘点计划列表
     */
...
//        }
    ...
    public function actionAddorupdate() 
    {
        ...
        if(Yii::$app->request->post()) {
            $result = $model->addOrUpdateCheckPlanning(Yii::$app->request->post(), CheckPlanningFlow::TYPE_WAREHOUSE);
            if($result["state"]) {
                $return["message"] = "新增成功  ";
                $return["type"] = "url";
                $return["url"] = Url::to(["warehousecheckplanning/index"]);
    ...
    public function actionVerify($id) 
    {
        ...
        $result = Flow::Verify(Flow::TYPE_CHECK_WAREHOUSE, $model, $remark);
    ...
    public function actionApproval($id)
    {
        $model = CheckPlanningFlow::findOne($id);
        $remark = Yii::$app->request->get('remark');
        $result = Flow::Approval(Flow::TYPE_CHECK_WAREHOUSE, $model, $remark);
        return Json::encode($result);
    }
    ...
    public function actionFinish($id)
    {
        $model = CheckPlanningFlow::findOne($id);
        $remark = Yii::$app->request->get('remark');
        $result = Flow::Finish(Flow::TYPE_CHECK_WAREHOUSE, $model, $remark);
        return Json::encode($result);
    }
    ...
    public function actionReject()
    {
        ...
        $model = CheckPlanningFlow::findOne($id);
        $result = Flow::Reject(Flow::TYPE_CHECK_WAREHOUSE, $model, $failCause);
        return Json::encode($result);
    }
    ...
    public function downloadIndex($query) {
        ...
        foreach ($all as $key => $val) {
            $nextStep = Flow::showNextStepByInfo(Flow::TYPE_CHECK_WAREHOUSE, $val);
            $datas[] = [
                'key' => $key+1,
                "name" => $val->name,
                "sn" => $val->sn,
                "warehouse" => Warehouse::getNameById($val->warehouse_id),
        ...
        $columns = [
            ...
            [ 'attribute' => 'warehouse','header' => '盘点仓库'],
        ...
        return Utils::downloadExcel($datas, $columns, "仓库盘点计划列表");
    }
    ...
    public function actionProof($id) {
        ...
        if(Yii::$app->request->post()) {
            ...
            if($result["state"]) {
                $return["message"] = "申请成功";
                $return["type"] = "url";
                $return["url"] = Url::to(["warehousecheckproof/index"]);
                return Json::encode($return);
            }
            $return['message'] = is_array($result["message"]) ? reset($result["message"]) : $result["message"];
            return Json::encode($return);
