...
class WarehousePlanning extends namespace\base\WarehousePlanning
{
    ...
    public function addOrUpdatePlanning($post) 
    {
        ...
        try{
            ...
            if(strtotime($this->planning_date) > strtotime($this->payment_term)) {
                $transaction->rollBack();
                return array("state" => 0, "message" => "采购时间不能大于付款时间");
            }
            ...
            BusinessRemind::disposeRemind($this->id, Flow::showTypeUrl($ftype), $this->status);
            ...
            if($this->buckle_amount > $totalMoney) {
                $transaction->rollBack();
                return array("state" => 0, "message" => "扣项不能大于采购总价");
            }
            //把金额的信息再次保存
    ...
    public function addOrUpdateExceptionPlanning($post)
    {
        ...
        $transaction = Yii::$app->db->beginTransaction();
        try{
            BusinessRemind::disposeRemind($this->id, Flow::showTypeUrl(Flow::TYPE_PLANNING_EXCEPTION), $this->status);
            $this->attributes = $post["WarehousePlanning"];
            if(strtotime($this->planning_date) > strtotime($this->payment_term)) {
                $transaction->rollBack();
                return array("state" => 0, "message" => "采购时间不能大于付款时间");
