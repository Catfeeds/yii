...
class WarehouseBuying extends namespace\base\WarehouseBuying
{
    ...
    public function Finish() {
        $oProcurement = OrderProcurement::findOne(["procurement_id" => $this->id]);
        if($oProcurement->status == Flow::STATUS_FINISH) {
            $buying = BusinessAll::findOne(["business_id" => $this->id, "business_type" => Flow::TYPE_BUYING]);
            if($buying) {
                $buying->is_complete = 1;
                $buying->save();
            }
            $oProcurementBusiness = BusinessAll::findOne(["business_id" => $oProcurement->id, "business_type" => Flow::TYPE_ORDER_FINANCE]);
            if($oProcurementBusiness) {
                $oProcurementBusiness->is_complete = 1;
                $oProcurementBusiness->save();
            }
        }
        ...
        foreach ($buyingProduct as $productItem) {
            ...
            if($stockItem) {
                $isCheck = CheckFlow::productIsCheckFlow($stockItem->id);
                if($isCheck) {
                    return array("state" => 0, "message" => $productItem->name."处于盘点中，无法执行入库");
                }
                ...
                $stockItem->purchase_price = $productItem->purchase_price;
                $stockItem->update();
                continue;
            }
    ...
    public function Reject() {
        ...
        if($orderProcurement->status == Flow::STATUS_FINISH && in_array($orderProcurement->pay_state, [OrderProcurement::PAY_STATE_DEPOSIT, OrderProcurement::PAY_STATUS_ALL])) {
            return ["state" => 0, "message" => "该订单已支付，无法驳回！请先入库，再执行退货收款流程。"];
        }
