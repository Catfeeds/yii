...
class FlowConfig extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            [['name', 'type'], 'required'],
            [['operation_role_id', 'operation_department_id', 'verify_role_id', 'verify_department_id', 'approval_role_id', 'approval_department_id', 'create_role_id', 'create_department_id', 'status', 'create_admin_id'], 'integer'],
            [['name'], 'string', 'max' => 255],
            [['operation_name', 'verify_name', 'approval_name', 'create_name'], 'string', 'max' => 12],
        ];
    }
