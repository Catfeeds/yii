    ...
    public function addTransferDep($post)
    {
        ...
        try{
            ...
            foreach ($post["stockId"] as $key => $stockId) {
                ...
                if($isCheck) {
                    $transaction->rollBack();
                    return array("state" => 0, "message" => $productItem->name."处于盘点中，无法申请转出");
                }
            ...
            if($totalAmount > 99999999) {
                $transaction->rollBack();
                return array("state" => 0, "message" => "转货总金额不能超过限制金额一亿");
            }
            if($num == 0) {
                $transaction->rollBack();
                return array("state" => 0, "message" => "请选择转货商品");
            ...
            $areaId = 0;
            $result = Flow::confirmFollowAdminId(Flow::TYPE_TRANSFEFDEP, $this, $totalAmount, $date, $areaId, $supplier, $meterialType);
            if(!$result["state"]) {
                $transaction->rollBack();
                return $result;
            }
