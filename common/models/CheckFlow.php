...
class CheckFlow extends namespace\base\CheckFlow 
{
    /**
     * 添加新的盘点校队
     * @param type $checkPlanningItem 盘点计划对象
     * @param type $checkInfo 盘点计划详情对象
                ...
                foreach ($post["checkStock"] as $dataId => $checkStock) {
                    ...
                    foreach ($checkStock as $pstockId => $stockNum) {
                        ...
                        if($stockNum > 99999999) {
                            throw new Exception("库存最大数量不能超过8位");
                        }
    ...
    public function Finish() {
        $checkPlanning = BusinessAll::findOne(["business_id" => $this->check_planning_id, "business_type" => [Flow::TYPE_CHECK_PLANNING, Flow::TYPE_CHECK_DEPARTMENT, Flow::TYPE_CHECK_WAREHOUSE]]);
        if($checkPlanning) {
            $checkPlanning->is_complete = 1;
            $checkPlanning->save();
        }
        $checkProof = BusinessAll::findOne(["business_id" => $this->id, "business_type" => [Flow::TYPE_CHECK_PLANNING_PROOF, Flow::TYPE_CHECK_DEPARTMENT_PROOF, Flow::TYPE_CHECK_WAREHOUSE_PROOF]]);
        if($checkProof) {
            $checkProof->is_complete = 1;
            $checkProof->save();
        }
        $amount = CheckFlowAmount::findAll(["check_flow_id" => $this->id]);
        foreach ($amount as $val) {
            $balanceLog = new DepartmentBalanceLog();
            $result = $balanceLog->addDepartmentBalanceLog($val->check_department_id, $val->id, DepartmentBalanceLog::BUSINESS_TYPE_AMOUNT_CHECK, DepartmentBalanceLog::MOD_CHECK, $val->check_amount, '盘点资金变动');
            if(!$result["state"]) {
                return $result;
            }
        }
...
//            $stockOutItem->number = $stockOutItem->number + $val->buying_number -  $val->product_number;
            $stockOutItem->number = $val->buying_number;
            $stockOutItem->save();
            //获取耗损除例外物料的其他物料ID
    ...
    public function Reject() {
        $checkPlanningItem = CheckPlanningFlow::findOne($this->check_planning_id);
        $checkPlanningItem->status = Flow::STATUS_UNION_REJECT;
        $checkPlanningItem->failCause = "盘点校对联合驳回盘点计划";
        if(!$checkPlanningItem->save()) {
            $message = $checkPlanningItem->getFirstErrors();
            return ["state" => 0, "message" => reset($message)];
        }
        $checkPlanning = BusinessAll::findOne(["business_id" => $checkPlanningItem->id, "business_type" => [Flow::TYPE_CHECK_PLANNING, Flow::TYPE_CHECK_DEPARTMENT, Flow::TYPE_CHECK_WAREHOUSE]]);
        if($checkPlanning) {
            $checkPlanning->status = Flow::STATUS_UNION_REJECT;
            $checkPlanning->is_complete = 1;
            $checkPlanning->save();
        }
        return ["state" => 1];
     ...
     */
    public static function productIsCheckFlow($pstockId) {
        $productList = CheckFlowProduct::findAll(["pstock_id" => $pstockId]);
        if(!$productList) {
            return false;
        }
        $flowIds = ArrayHelper::getColumn($productList, "check_flow_id");
        $checkItem = CheckFlow::findOne(["id" => $flowIds, "status" => [Flow::STATUS_APPLY_VERIFY, Flow::STATUS_APPLY_APPROVAL, Flow::STATUS_APPLY_FINISH]]);
        if($checkItem) {
            return true;
        }
        return false;
     ...
     * @return boolean true 是 false 否
     */
    public static function amountIsCheckFlow($departmentId) {
        $amountList = CheckFlowAmount::findAll(["check_department_id" => $departmentId]);
        if(!$amountList) {
            return false;
        }
        $flowIds = ArrayHelper::getColumn($amountList, "check_flow_id");
        $checkItem = CheckFlow::findOne(["id" => $flowIds, "status" => [Flow::STATUS_APPLY_VERIFY, Flow::STATUS_APPLY_APPROVAL, Flow::STATUS_APPLY_FINISH]]);
        if($checkItem) {
            return true;
        }
