...
class Config extends namespace\base\Config
{
    ...
    public function setCompanyInfo($post) {
        ...
        try{
            ...
            Yii::$app->cache->set('logo', $logoModel->set_value, 86400);
            Yii::$app->cache->set('company', $companyModel->set_value, 86400);
            Yii::$app->cache->set("company_".Admin::getDepId(), $companyModel->set_value . " -- " . Department::getNameById(Admin::getDepId()), 86400);
            Admin::updateAll(["is_first" => 1], "id = " . Yii::$app->user->getId());
    ...
    public function setAllConfig($post) {
        ...
        try{
            ...
            foreach(Flow::getTypeSelectData() as $type => $name){
                ...
                if($post["flowType"][$type] > 365) {
                    throw new Exception($name."的天数必须小于365天");
                }
